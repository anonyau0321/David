...
public class ReadFile {
	...
	public static void main(String[] args) throws IOException
	{
		...
		for(int i = 0; i < bench2.size(); i++) {//hsqldb6, lusearch6, xalan6, jython9, luindex9, luearch9-fixed, pmd9, xalan9
			...
			String atomic_out = "atomic_specification/DV_iteration/" + bench2.get(i) + "_" + trace;
			...
			PrintStream ps = new PrintStream(new FileOutputStream("DV_iteration_"+bench2.get(i)+trace));
			System.setOut(ps);
			...
			while(true)
			{
				...
				if(lastLen == newLen)
				{
					...
					fileOutput3.flush();
					System.out.println("Distinct Trans: " + transGraph.TRANSCNT);
		...
		for(int i = 0; i < bench1.size(); i++) {//eclipse6, sunflow9
			String bench_trace1 =  loc + "trace_" + trace + trace_path + bench1.get(i) + "1.log"; //recorded trace file
			...
			String atomic_out = "atomic_specification/DV_iteration/" + bench1.get(i) + "_" + trace;
			FileWriter f4 = new FileWriter(atomic_out, true);
			BufferedWriter fileOutput4 = new BufferedWriter(f4);
			TransactionalHBGraph transGraph = new TransactionalHBGraph();
			PrintStream ps = new PrintStream(new FileOutputStream("DV_iteration_"+bench1.get(i)+trace));
			System.setOut(ps);
			byte[] readBytes1 = new byte[BUFFER_SIZE];
